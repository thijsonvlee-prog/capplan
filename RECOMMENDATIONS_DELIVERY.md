...
- **Proposed improvement:** Add a comment in `ImportSourceManager.tsx` referencing `api-import-helpers.ts` as the source of truth, or create a shared `TARGET_ENTITY_OPTIONS` constant with labels that both sides can derive from.
...
- **API credentials storage is plaintext JSON:** The `apiCredentials` field stores credentials as JSONB. Adequate for Phase 1 but should be reviewed before production use with real API keys.
- **PerformanceEvent table growth:** Table grows unbounded. `cleanupOldEvents()` exists but is never called.
- **API import timeout:** The 30-second timeout on external API requests may be insufficient for slow APIs. Vercel serverless function timeout is the binding constraint.
- **API response size unbounded:** No limit on response body size from external APIs. MAX_ROW_COUNT check only applies after parsing.
...
- **Auth env vars required for deployment:** Without `NEXTAUTH_SECRET`, auth is inactive and role enforcement is skipped silently.
...
- **Split ImportSourceManager.tsx (~1477 lines):** Large but well-structured with distinct sections. Splitting would fragment the connectivity hub feature.
...
- **Other `.find()` calls in render paths:** ScenarioSelector, RosterAssigner, etc. operate on small arrays (<10 items). Negligible.
...
- **Limit API response body size:** Could add a streaming size check, but MAX_ROW_COUNT provides sufficient protection after parsing.
- **Extract execute result UI into shared component:** The CSV and API execute result displays are similar but not identical. Only 2 consumers.
- **Add React.memo to CapacityTable/CapacityKPIs:** These render infrequently. Internal `useMemo` is sufficient.
- **Extract SortIcon from PlanningGrid:** Tiny inline function. Moving outside reduces locality.
- **Centralize Map construction across components:** Each component constructs Maps from its own fetched data. A shared hook adds coupling without benefit.
- **Scoped cache invalidation in useApi:** Current global invalidation is simple and reliable at current data volumes.
- **Add error states to capacity/planning components:** Parent pages handle loading/error states. Redundant.
- **Extract useDebounce to custom hook:** Only used in DriverList with a simple inline implementation. One consumer.
- **Add aria-labels to chart visualizations:** Recharts manages its own accessibility.
- **Batch FK validation in driver creation:** Already uses `Promise.all()`. Marginal improvement for complexity.
- **Deduplicate date-parsing logic:** Completed as PB-183.
- **Move COMPARE_COLORS / remove unused types / clean mobile CSS:** Completed as PB-176/177/178.
- **Preferences `key` field length cap:** Near-zero risk since keys are hardcoded strings in the frontend.
- **useApi cache key uses Function.toString():** Theoretically fragile under minification, but different arrow functions produce different minified strings because URL strings differ.
- **Import logs route sequential queries:** Two sequential queries. P4 Low, same category as DE-REC-059.
- **autoCloseOpenRecords UTC dependency:** `toISOString()` always returns UTC. Not a defect.
- **Scenario DELETE preference cleanup for auth users:** Completed as PB-184.
- **Auth checks on scenarios/roster-profiles/sub-record GET endpoints:** Completed as PB-185.
- **Date format validation on sub-record routes:** Completed as PB-186.
